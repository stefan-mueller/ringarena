...
Mit diesem Datensatz lässt sich nun der "Heimvorteil" berechnen (genauer beschrieben in einem [Paper von Liam Kneafsey und mir](https://muellerstefan.net/papers/ijpas_kneafsey_mueller.pdf), das bald im _International Journal of Performance Analysis in Sport_ veröffentlicht wird). Grob gesprochen kann der Wert für jedes Team in der Saison zwischen 0 und 1 liegen. Ein Wert von 1 bedeutet, dass alle Punkte daheim eingefahren wurde, eine 0 impliziert, dass ein Team Punkte lediglich auswärts errungen hat. Folglich markieren alle Werte über 0.5 einen Heimvorteil: das Team hat mehr Heim- als Auswärtspunkte gesammelt.
...
gg_home_density <- function(data = sis_home_advantage, 
  ...
  data %>% 
    ...
    labs(x = "Heimvorteil", y = NULL,
         title = "Die Verteilung des Heimvorteilsrankings für TSV-Teams und die jeweiligen Gegner", 
...
ggplot(data = sis_long_tsv, aes(x = year, y = goals_team_match, colour = match_type, shape = match_type)) +
  ...
  labs(x = "Saison", 
       ...
       subtitle = "Graue Flächen zeigen Konfidenzintervalle; Saisonplatzierung und Ligaabkürzung im oberen Teil der Grafik.")
```
